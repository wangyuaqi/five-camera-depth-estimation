...
/*void LRCheckOut(cv::Mat& left_img,cv::Mat& right_image,cv::Mat_<float>& disp_mat,cv::Mat& dis_result){
    ...
    for(int col = 0;col<src_cols;col++) //x
    {
        for(int row =0; row<src_rows;row++){//y
            cv::Vec3b src_color = left_img.at<cv::Vec3b>(row,col);

            int right_col = col-disp_mat(row,col);
            if(right_col<0){
                dis_result.at<uchar>(row,col)=255;
            }else{
                cv::Vec3b right_color = right_image.at<cv::Vec3b>(row,right_col);
                int sum_dif =0;
                sum_dif+=(abs(right_color[0]-src_color[0])+abs(right_color[1]-src_color[1])+abs(right_color[2]-src_color[2]));
